...
enum PmxSoftBodyFlag : uint8_t
{
	BLink = 0x01,
	Cluster = 0x02,
	SLink = 0x04	//class Linkと名前が被ってて使えないからSLinkに
};
...
public:
	...
	int textureCount;
	// テクスチャ名前配列
	std::unique_ptr< std::wstring []> textureName;
	//テクスチャ配列
...
public:
	...
	int rigidBodyCount;
	// 剛体配列
	std::unique_ptr<PmxRigidBody []> rigidBodies;
	// ジョイント数
	int jointCount;
	// ジョイント配列
